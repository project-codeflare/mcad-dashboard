...
class AllNamespaces {
  async get() {
    ...
    for(let i=0;i<jsonStringList.length;i++) {
      ...
      if(jsonData.metadata.creationTimestamp) {
        ...
        timestr = (day).toString() + "d " + (hour).toString() + "h " + (minutes).toString() + "m " + Math.floor(seconds).toString() + "s";
      }else {
        timestr = 'Not found';
      }
      ...
      const wrapperKey:string = jsonData.metadata.name.toString() + jsonData.metadata.namespace.toString();
  ...
  pullJobs(): Promise<string>{
    return new Promise((resolve, reject) => {
      ...
      const child = spawn('sh', ['./backend/src/appwrapper_puller.sh'], {stdio: 'pipe', buffer: true});
